//! php's SQLSTATE descriptions (`ext/pdo/pdo_sqlstate.c`): the text a
//! `PDOException` message opens with, `SQLSTATE[HY000]: General error: …`.

/// `(sqlstate, description)`, sorted by state.
pub static SQLSTATES: &[(&str, &str)] = &[
    ("00000", "No error"),
    ("01000", "Warning"),
...
/// The description of a SQLSTATE, `<<Unknown error>>` for one php does not list.
pub fn describe(state: &str) -> &'static str {
    SQLSTATES
        .binary_search_by(|(s, _)| s.cmp(&state))
        .ok()
        .map_or("<<Unknown error>>", |i| SQLSTATES[i].1)
}
