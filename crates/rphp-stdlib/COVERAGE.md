...
| socket (`socket.rs`) | 19 | S12: the **transports** — `stream_socket_client`/`server`/`accept`/`get_name`/`sendto`/`recvfrom`/`shutdown`/`pair` over `tcp`/`udp`/`unix`/`udg`, `fsockopen`/`pfsockopen`, `stream_set_timeout` (+`socket_set_timeout`), `stream_get_transports`/`stream_get_wrappers`, and the host lookups `gethostname`/`gethostbyname`/`gethostbyaddr`/`gethostbynamel`. A socket is an ordinary stream: it lands in the same descriptor slot a `proc_open` pipe uses, so `fread`/`fwrite`/`fgets`/`stream_get_line`/`stream_select`/`fclose` need nothing of their own. php's own shapes are kept — a socket's metadata carries **no** `wrapper_type` (a transport is not a wrapper) and a `stream_socket_pair()` end no `uri`; `feof()` polls and peeks a byte rather than waiting for a read to come back short; a read timeout is `timed_out` with `eof` still false. tier-a: `net/socket-{tcp,dgram,errors,timeout}.php`, `net/host-lookup.php` |
| http (`http.rs`) | 3 | S12: the **`http://` wrapper** behind `fopen`/`file_get_contents`/`file`/`readfile`, plus `get_headers`, `http_get_last_response_headers` and `http_clear_last_response_headers`. The `http` context options php reads (`method`, `header` as string or array, `content`, `user_agent`, `protocol_version`, `follow_location`, `max_redirects`, `ignore_errors`, `timeout`), redirects followed with every hop's headers accumulating, the `Content-type not specified` notice for a body sent without one, `allow_url_fopen=0` refused with php's pair of warnings, and a 4xx/5xx that php will not open quoting the status line `\r` and all. The headers reach the script all three ways php fills: `wrapper_data`, `http_get_last_response_headers()` and `$http_response_header`. The body stays on the live socket — what the wrapper read past the headers is php's `unread_bytes`. Redirects follow php's method rules (301/302/303 become a `GET` with the body and its content headers dropped; 307/308 repeat the request). `https://` works through the `ssl` transport (`tls.rs`). tier-a: `net/http-wrapper.php` (which starts a second copy of the running interpreter as its server, so each engine is tested against itself) |
| tls (`tls.rs`) | — | S12: the **`ssl://`/`tls://` transports** behind `stream_socket_client`, `stream_socket_enable_crypto` and `https://`, on **rustls** with the **ring** provider and the platform trust store (the one php's openssl reads). php's `ssl` context options that decide whether a connection happens are honoured — `verify_peer`, `verify_peer_name`, `allow_self_signed`, `cafile`, `peer_name` — and match php case for case: expired, self-signed, wrong-host and untrusted-root are all refused by default and all accepted once the matching option is off. `stream_socket_enable_crypto` upgrades a connected tcp stream in place (SMTP's `STARTTLS`), with php's `ValueError` when `$crypto_method` is missing and php's `This stream does not support SSL/crypto` for a stream that has no socket under it. **The server side** (2026-09-23): a `tls://`/`ssl://` listener handshakes each connection at `stream_socket_accept()` with the listener's `local_cert` (key from `local_pk`, or from the same file), accepted streams inherit the listener's context, and `stream_socket_enable_crypto(…, STREAM_CRYPTO_METHOD_*_SERVER)` upgrades an accepted tcp stream; a listener with no certificate, a missing chain file and a missing key file each fail with php's exact warnings. `stream_get_meta_data()` reports `crypto` (protocol and openssl's cipher name). Tested against a real TLS server by `tools/rphp/tests/tls.rs` (a `rustls` server on loopback presenting `fixtures/tls`, run under both engines, and the reverse: each engine's server script pinged by a rustls client, five modes); the offline half is tier-a `net/tls-offline.php`. **Not implemented:** turning crypto back off, `capture_peer_cert*`, `ciphers`, `passphrase`, `peer_fingerprint`, and `SNI_enabled => false`. |
| random objects (randomizer.rs) | 0 fn / 10 classes | S2: `Random\Engine`, `Random\CryptoSafeEngine`; engines `Mt19937` (both modes, own state — not `mt_rand()`'s), `PcgOneseq128XslRr64` (`jump`), `Xoshiro256StarStar` (`jump`/`jumpLong`), `Secure` (CSPRNG, uncloneable, not serializable) — byte-exact `generate()`, php's state arrays in `__serialize`/`__debugInfo`, validated `__unserialize`, seed/mode errors; `Random\Randomizer` (readonly `$engine`, default `Secure`): `getInt` (php's range stitching + 50-attempt rejection; `MT_RAND_PHP` bad scaling), `nextInt`, `nextFloat`, `getFloat` (γ-section, all four `IntervalBoundary` cases), `getBytes`, `getBytesFromString`, `shuffleArray`, `shuffleBytes`, `pickArrayKeys`, serialization; user engines of any output width (`BrokenRandomEngineError` on `""`); `RandomError`/`BrokenRandomEngineError`/`RandomException`; enum `IntervalBoundary` (embed prelude). Gap: `pickArrayKeys($a, 1)` on arrays with holes (bucket sampling not modelled). tier-a: `random/{engines,randomizer,float,user-engine,serialize,errors,classes}.php` |
| posix / pcntl (`rphp-ext-posix`) | 40 + 23 | 2026-09-23: every `posix_*` php 8.5 has on darwin (passwd/group entries, rlimits with "unlimited", sysconf/pathconf/fpathconf, isatty/ttyname over ints and streams, php's errno convention) and `pcntl_*`: signal/dispatch/async_signals (async: the OS handler pokes the interrupt, `Interp::safepoint` runs `poke_hook`, also after every native call), sigprocmask, alarm, fork/wait/waitpid/waitid + the status macros, exec, priorities. `unsafe` confined to `sys.rs`; pcntl is registered for CLI/embed only (php builds it for the CLI alone, and fork is only sound single-threaded). **Not implemented:** `pcntl_{get,set}qos_class` + `Pcntl\QosClass`, the Linux-only functions; `sleep()` is not cut short by a signal. tier-a: `posix/*.php`, `pcntl/*.php` |
| xml, xmlwriter, xmlreader (`rphp-ext-xml`) | 22 + 42 + 2 classes | 2026-09-23: **ext/xml** over a replica of libxml2's push parser as php's expat-compat layer drives it (chunk consumption, `xmlParseCharData` text splitting, handler line/column/byte index, libxml error numbers + the compat layer's 102 strings, case folding, UTF-8/ISO-8859-1/US-ASCII targets, `_ns` mode, DTD notation/unparsed/external-entity handlers, `xml_parse_into_struct()`, callable or `xml_set_object()` handlers with the 8.4/8.5 deprecations). **XMLWriter** replicates `xmlTextWriter` (states, indentation, LIFO namespace declarations, escaping, output encodings, DTD declarations, memory/file/stream targets with 4000-byte buffering, `toMemory`/`toUri`/`toStream`, the procedural API). **XMLReader** walks the rphp-ext-dom tree in xmlTextReader order with the attribute cursor, `next()`, inner/outer XML, `expand()` and libxml's 3-line diagnostics. **Not implemented:** static `XMLReader::open()`/`XML()` (instance-only: native statics get no called class), schema validation (`isValid()` is false), UTF-16/iconv encodings, chunked-reader error timing. tier-a: `xml/*.php` (13) |
| bcmath (`rphp-ext-bcmath`) | 14 + `BcMath\Number` | 2026-09-23: every bc function (`bcadd`…`bcround`, `bcdivmod`, `bcscale` over the `bcmath.scale` ini) and php 8.4's `BcMath\Number`. Numbers are libbcmath's `bc_num` ported field for field (sign, integer length, scale, signed zeros), so results are truncated, padded and signed exactly as php's; products and quotients run on base-10⁹ limbs. `Number` has virtual readonly `value`/`scale`, php's automatic result scales, dumps, clone and (un)serialize, and takes part in operators through the runtime's **`NativeOperators` class hook** (php's `do_operation` + `compare`), consulted only off the numbers fast path. `%` and the bitwise operators now read operands like `zendi_try_get_long`. Gaps: `foreach` over a Number lists its props; `empty($n->value)` for `"0.00"`; the float-operand deprecation inside `sort()`; out-of-memory-sized powers are a ValueError. tier-a: `bcmath/*.php` (7) |
| zlib (`zlib.rs`, `zlib/`) | 29 | 2026-09-23: `gzcompress`/`gzdeflate`/`gzencode`/`zlib_encode` and their decoders (php's inflate rounds with the 100-round limit, `max_length`, `zlib_decode` detection with raw fallback), `deflate_init`/`deflate_add` (every flush mode, reset after `ZLIB_FINISH`, `level`/`memory`/`window`/`strategy`/`dictionary`), `inflate_init`/`inflate_add`/`inflate_get_status`/`inflate_get_read_len`, final `DeflateContext`/`InflateContext`, `gzopen` + the gz handle functions, `gzfile`, `readgzfile`, `compress.zlib://` behind `fopen`/`file_get_contents`/`file_put_contents`/`file`/`readfile`, and the `zlib.deflate`/`zlib.inflate` stream filters. **Compression is a line-for-line safe port of zlib 1.2.12's `deflate.c`/`trees.c`** (`zlib/deflate.rs`): php passes memLevel 9 and script-chosen memLevel/strategy, which flate2 cannot, and the bytes must be php's; tested against the system libz and against php over a 160-stream seeded sweep. Inflate is flate2. gz handles are ordinary `stream` resources. **Not implemented:** `ob_gzhandler`, `zlib.output_compression` (ini registered, never compresses — php's CLI answer), `inflate_add(ZLIB_BLOCK)` stopping at block boundaries, gz handles' `fstat`/`ftruncate`, `compress.zlib` in `stream_get_wrappers()`. tier-a: `zlib/*.php` (11) |
| standard tail (`standard_tail.rs`, `ini_parse.rs`, `image.rs`) | ~75 | 2026-09-23/24: `parse_ini_string`/`parse_ini_file` (php's INI scanner + its bison tables: exact syntax errors, NORMAL/RAW/TYPED, sections, arrays, constants, `${var}` env-only, operators); `getimagesize`/`getimagesizefromstring` (GIF/JPEG+APPn/PNG/SWF/SWC/PSD/BMP/TIFF/JPC/JP2/IFF/WBMP/XBM/ICO/WEBP/AVIF/HEIF/SVG), `image_type_to_*`, `IMAGETYPE_*`, `iptcparse`/`iptcembed`; `fnmatch`, `getopt`, `md5_file`/`sha1_file`, `str_increment`/`str_decrement`, `ini_parse_quantity`, `forward_static_call(_array)`, the `strchr`/`socket_get_status`/`socket_set_blocking`/`set_file_buffer` aliases, `stream_supports_lock`, `assert_options`, `strptime` (macOS libc port), `hebrev`, `get_meta_tags`, `php_strip_whitespace`, `dl`, `get_browser`, `realpath_cache_*`, `request_parse_body`, `output_*_rewrite_var(s)`, `register/unregister_tick_function` (never fire: no `declare(ticks)`), `sys_getloadavg`, `cli_*_process_title` (ps title not rewritten); `getserv*`/`getproto*`, `nl_langinfo` (C locale), `getrusage`, `proc_nice`, `time_sleep_until`, `ftok`, `lchown`/`lchgrp`, `net_get_interfaces`, `mail()` (sendmail_path), `dns_get_record`/`checkdnsrr`/`dns_check_record`/`getmxrr`/`dns_get_mx` (own resolver over /etc/resolv.conf). **Not implemented:** `debug_zval_dump` (refcounts are not php's), `header_register_callback` (needs a first-output hook). tier-a: `standard/*.php` |
| user stream wrappers & popen (`stream_wrappers.rs`) | 6 | 2026-09-23/24: `stream_wrapper_register`/`stream_register_wrapper`/`unregister`/`restore` over php's wrapper table (built-ins removable and restorable, `STREAM_IS_URL`, `allow_url_include`). A user stream is php's buffered stream over the object: 8192-byte `stream_read` fills each followed by `stream_eof`, chunked writes, in-buffer seeks, owed flush at close, php's exact missing-method warnings; every handle function incl. `stream_select` via `stream_cast`; the path functions (`url_stat` with QUIET/LINK and the stat slot, unlink/rename/mkdir/rmdir, `stream_metadata`, copy); `dir_*` for opendir/readdir/scandir/`dir()`; **include/require through a wrapper** (runtime `IncludeStreamHook`). `popen`/`pclose` via `/bin/sh` under the SpawnPolicy. **Gaps:** close on unset happens at request end (no refcounted resources); stream filters, `fscanf` and the csv functions on user streams; the default-context id shifts after a registration. tier-a: `wrappers/*.php` |
| Core/date/hash/SPL/filter tails (`core_tail.rs`, date, hash, spl_autoload, filter) | ~25 | 2026-09-23/24: Core — `exit`/`die` as functions (8.4: `exit(...)` lowers to a call to `\exit`), `clone()` with `$withProperties` (8.5), `get_included_files`/`get_required_files`, `get_declared_traits`, `get_mangled_object_vars`, `get_resource_id`. date — `date_parse_from_format` (timelib's from-format loop, shared with `createFromFormat`), `date_sun_info`/`date_sunrise`/`date_sunset` (astro.c port, bit-exact doubles, deprecations, `date.*` ini), `timezone_location_get`, `timezone_version_get` (system tzdata in php's `YYYY.N` form). hash — `hash_update_file`, `hash_update_stream`, `mhash*`, `MHASH_*`. SPL — `spl_autoload`, `spl_autoload_extensions`, `spl_classes`. filter — `filter_input`/`filter_input_array`/`filter_has_var` over the SAPI's original input arrays. **Gaps:** mhash ids of algorithms rphp lacks (haval/tiger/gost/snefru/murmur3) throw; `spl_classes()` lists three classes rphp has not got. tier-a: `{core,date,hash,spl,filter}/*-tail.php` |
| mbstring regex half (`mbregex.rs`) | 19 | 2026-09-23/24: the `mb_ereg*` family, `mb_split`, `mb_ereg_match`, `mb_regex_encoding`, `mb_regex_set_options`, the `mb_ereg_search_*` cursor, `mb_convert_kana`, `mb_send_mail`. Oniguruma patterns (Ruby default; Perl/Java/GNU/POSIX/grep/emacs flags) are **translated to PCRE2** with Oniguruma's compile-error texts and its name order; regex encodings convert to UTF-8 with offsets mapped back. Rejected with an "…not supported by rphp" warning rather than matched wrongly: `\y`/`\Y`, the `l` option, level backrefs, `(?~|…)`; unbounded lookbehind and repeats above 65535 fail to compile; stack/retry limits map to PCRE2 limits with other thresholds. tier-a: `mbstring/{ereg*,split_match,regex_*,kana,send_mail}.php` |
| intl: Transliterator, break iterators, Spoofchecker, UConverter (`rphp-ext-intl`) | — | 2026-09-24: `Transliterator` + `transliterator_*` on ICU4X's rule engine with ICU's ID grammar and a registry dumped from php 8.5 / ICU 78.3 (Han-Latin compiled from CLDR 48 source; Any-Title/Hex/Name/<script> in code; `createFromRules`) — 753/758 ids byte-exact on a mixed-script corpus, Symfony's slugger exact. `IntlBreakIterator` family (word/line/sentence/character/title, rule-based, code point, parts) on ICU4X segmenters with ICU's cursor semantics and rule statuses. `Spoofchecker` (UTS #39, Unicode 17 confusables). `UConverter` over ICU's dumped registry, the Unicode forms, Latin-1/ASCII and 150 SBCS tables with ICU's callback protocol. **Gaps:** custom/compiled break rules and `getBinaryRules`; Japanese dictionary splits; `Latin-Hebrew`/`ug-ug_FONIPA`; `Any-FCD`/`FCC`; MBCS/ISO-2022/UTF-7/SCSU/BOCU-1 converters and the CLOSE/CLONE callbacks. tier-a: `intl/{translit,breakiter,spoof,uconverter}*.php` |
| intl: MessageFormatter, IntlListFormatter, IntlDatePatternGenerator, ResourceBundle (`rphp-ext-intl`) | — | 2026-09-24: `MessageFormatter` + `msgfmt_*` — a port of ICU's MessagePattern/MessageFormat (apostrophe mode, php's parse-error texts), number/date/time styles, patterns and skeletons, choice, plural/selectordinal (ICU 78 plural rules dumped from the oracle), select, nesting, spellout/ordinal/duration through a new **RBNF engine** over the oracle's rules; `parse()` for text/number/date/choice — what Symfony's IntlFormatter drives. `IntlListFormatter` (every locale × type × width), `IntlDatePatternGenerator` (port of ICU's generator; 140 skeletons × 906 locales match but 3 locales). `ResourceBundle`: `getLocales()` only. **Gaps:** ResourceBundle bundle access (needs ICU's locale data tree), RBNF parsing, number skeletons with units/compact/permille, NumberFormatter SPELLOUT/ORDINAL/DURATION not yet wired to the RBNF engine. tier-a: `intl/msgfmt*.php`, `intl/listfmt.php`, `intl/datepatgen.php`, `intl/resbundle-locales.php` |
| filters (`filters.rs`, `filters/convert.rs`, `user_filters.rs`, `dechunk.rs`) | 10 | S12 + 2026-09-23: **php's bucket model** — `stream_filter_append/prepend/remove`, `stream_get_filters`, `stream_filter_register` (exact and `prefix.*` names), `php_user_filter`, `StreamBucket`, `stream_bucket_make_writeable/append/prepend/new`, `PSFS_*`. `fwrite()` answers the first filter's consumed count; reads pull 8192-byte chunks; `fflush`/seek flush with `FLUSH_INC`, `fclose`/remove/request end with `FLUSH_CLOSE` (and `onClose()`). Built-ins: `string.rot13/toupper/tolower`, `consumed`, `dechunk` (incremental, shared with the `http://` wrapper — what Symfony's `NativeHttpClient` attaches), `convert.base64-*`, `convert.quoted-printable-*` (every option, safe at any write boundary), `convert.iconv.*`, `zlib.deflate`/`zlib.inflate`. `php://filter/read=…/write=…/resource=…` for fopen, file_get_contents, file_put_contents, file and readfile. **Not implemented:** `bzip2.*`, `php://filter` in `include`. Divergences: a filtered stream is closed at request end rather than when its last reference goes (no refcounted resources); memory-stream "pre-buffered data" is not modelled. tier-a: `file/stream-filters.php`, `filters/*.php` |
...
(E5), `getrusage` (needs libc), `phpcredits`.
