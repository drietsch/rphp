...
| filters (`filters.rs`, `dechunk.rs`) | 4 | S12: **stream filters** — `stream_filter_append`/`prepend`/`remove` and `stream_get_filters`, with the chain a write passes through on the way in (`fwrite` still answering the bytes it was *given*, as php does) and a read on the way out. The filters are the ones needing nothing but this crate: **`dechunk`**, `string.rot13`, `string.toupper`, `string.tolower`, `consumed`. `dechunk` is an incremental decoder (`dechunk.rs`, property-tested over every split of a body) shared with the `http://` wrapper, which needs it for a chunked response; it is also exactly what Symfony's `NativeHttpClient` attaches to its buffer, and the last thing that stood between rphp and a working HttpClient. **Not implemented:** `zlib.*`, `bzip2.*`, `convert.*` (including `convert.iconv.*`), and user filters (`stream_filter_register`, `php_user_filter`, `stream_bucket_*`). tier-a: `file/stream-filters.php` |
