...
**Registry size: 171 rows** (wave 1: ~103 distinct functions; wave 2: +11 by-reference
builtins). Plus aliases and the initial slice.
...
- **callables/closures** — higher-order functions (still pending).
...
| array     | 18 + 11 | value-returning: `array_slice/flip/unique/diff/intersect/combine/chunk/column/fill/pad/search/product/…`, `array_is_list`. **By-ref (wave 2):** `sort/rsort/asort/arsort/ksort/krsort`, `array_push/pop/shift/unshift`, `array_splice` |
...
| pcre      | 5  | `preg_quote`, `preg_match` (**now incl. by-ref `$matches`**), `preg_replace`, `preg_split`, `preg_grep` — over PCRE2 |
