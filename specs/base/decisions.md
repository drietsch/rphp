...
*Amendment (2026-09-21, as built):* the built-in server keeps one long-lived thread per worker (`PHP_CLI_SERVER_WORKERS`) serving its connections in turn rather than a thread per connection, because the compiled-unit cache is **per thread** — a compiled `Module` holds `Rc`-counted string constants, so the cache cannot be the `Send + Sync` `Engine` field the decision names; `rphp -b host:port` (`rphp-sapi-fcgi`, php-cgi's flag with `PHP_FCGI_CHILDREN` workers) is the FastCGI SAPI, measured against `php-fpm -n` rather than php-cgi, and the request binding it shares with `-S` lives in `rphp-embed::cgi` (php's `$_SERVER` layout stays each SAPI's own).
...
**Status.** Proposed (2026-09-22, for guardian-runner roadmap M1). Places the `openssl` of ADR-032. **Affected:** `08-stdlib-ext.md` (the extension roster).
