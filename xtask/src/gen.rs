...
//! PHP oracle manifest (`manifest/php-8.5.10`, produced by
...
mod tests {
    ...
    fn manifest() -> Manifest {
        let root = workspace_root();
        Manifest::load(&root.join("manifest/php-8.5.10")).expect("manifest loads")
    }
    ...
    fn generates_ctype_snapshot() {
        ...
        let files = generate(&ext, "manifest/php-8.5.10").unwrap();
    ...
    fn generates_core_details() {
        let m = manifest();
        let files = generate(&ExtData::collect(&m, "Core"), "manifest/php-8.5.10").unwrap();
        ...
        let date = generate(&ExtData::collect(&m, "date"), "manifest/php-8.5.10").unwrap();
        assert!(date["consts.rs"].contains("ConstDef { name: \"SUNFUNCS_RET_STRING\", value: ConstValue::Int(1), deprecated: true },"));
        let json = generate(&ExtData::collect(&m, "json"), "manifest/php-8.5.10").unwrap();
        ...
        let reflection =
            generate(&ExtData::collect(&m, "Reflection"), "manifest/php-8.5.10").unwrap();
    ...
    fn every_extension_generates_deterministically() {
        ...
        for name in m.extensions.keys() {
            ...
            let a = generate(&ext, "manifest/php-8.5.10").unwrap_or_else(|e| panic!("{name}: {e}"));
            let b = generate(&ext, "manifest/php-8.5.10").unwrap();
    ...
    fn generated_code_compiles() {
        ...
        for name in ["Core", "ctype", "json"] {
            ...
            for (file, content) in generate(&ext, "manifest/php-8.5.10").unwrap() {
